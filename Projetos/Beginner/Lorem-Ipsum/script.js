...
form.addEventListener("submit", e => {
    ...
    // Determinando o que retornara do array "loremText" com slice([inicio [,fim]]);
    const value = parseInt(numOfLorem.value);
    let allText = loremText.slice(0, value);
    
    // Inserindo a quantidade de paragrafos de acordo com o valor informado pelo usuario
    allText = allText.map(item => item).join("");
    result.innerHTML = allText;

    // Copiando texto Gerado
...
// Sincronizando valores dos Input: Number e do Range
...
// Passando as alteraçoes dos valores para os inputs dinamicamente
